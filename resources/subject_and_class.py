...
class Subject_And_Class(Resource):
    ...
    parser.add_argument("exam_date", type=str, required=True, help=help)

    @gv_authenticate
    def post(self):
        data = Subject_And_Class.parser.parse_args()
        if Subject_And_ClassModel.find_row(data["class_id"], data["subject_id"]):
    ...
    def get(self, class_id=None, page=None, per_page=None):
        ...
        if class_id is None:
            ...
            return {"list": list, "count": len(Subject_And_ClassModel.query.all())}, 200

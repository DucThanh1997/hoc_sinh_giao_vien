...
class PythonTimeBuilder(BaseTimeBuilder):
    ...
    def build_time(cls, hh=None, mm=None, ss=None, tz=None):
        ...
        if mm is not None:
            if "." in mm:
                ...
                minutes = cls.cast(mm, int, thrownmessage="Invalid minute string.")
        ...
        if ss is not None:
            if "." in ss:
                ...
                seconds = cls.cast(ss, int, thrownmessage="Invalid second string.")

        hours, minutes, seconds, microseconds = PythonTimeBuilder._distribute_microseconds(
            microseconds,
    ...
    def build_duration(
        ...
        if PnY is not None:
            if "." in PnY:
                years, remainingmicroseconds = cls._split_to_microseconds(
                    PnY, MICROSECONDS_PER_YEAR, "Invalid year string."
                )
                microseconds += remainingmicroseconds
            else:
                years = cls.cast(PnY, int, thrownmessage="Invalid year string.")

        if PnM is not None:
            if "." in PnM:
                months, remainingmicroseconds = cls._split_to_microseconds(
                    PnM, MICROSECONDS_PER_MONTH, "Invalid month string."
                )
                microseconds += remainingmicroseconds
            else:
                months = cls.cast(PnM, int, thrownmessage="Invalid month string.")

        if PnW is not None:
            if "." in PnW:
                weeks, remainingmicroseconds = cls._split_to_microseconds(
                    PnW, MICROSECONDS_PER_WEEK, "Invalid week string."
                )
                microseconds += remainingmicroseconds
            else:
                weeks = cls.cast(PnW, int, thrownmessage="Invalid week string.")

        if PnD is not None:
            if "." in PnD:
                days, remainingmicroseconds = cls._split_to_microseconds(
                    PnD, MICROSECONDS_PER_DAY, "Invalid day string."
                )
                microseconds += remainingmicroseconds
            else:
                days = cls.cast(PnD, int, thrownmessage="Invalid day string.")

        if TnH is not None:
            if "." in TnH:
                hours, remainingmicroseconds = cls._split_to_microseconds(
                    TnH, MICROSECONDS_PER_HOUR, "Invalid hour string."
                )
                microseconds += remainingmicroseconds
            else:
                hours = cls.cast(TnH, int, thrownmessage="Invalid hour string.")

        if TnM is not None:
            if "." in TnM:
                minutes, remainingmicroseconds = cls._split_to_microseconds(
                    TnM, MICROSECONDS_PER_MINUTE, "Invalid minute string."
                )
                microseconds += remainingmicroseconds
            else:
                minutes = cls.cast(TnM, int, thrownmessage="Invalid minute string.")

        if TnS is not None:
            if "." in TnS:
                seconds, remainingmicroseconds = cls._split_to_microseconds(
                    TnS, MICROSECONDS_PER_SECOND, "Invalid second string."
                )
                microseconds += remainingmicroseconds
            else:
                seconds = cls.cast(TnS, int, thrownmessage="Invalid second string.")
    ...
    def build_repeating_interval(cls, R=None, Rnn=None, interval=None):
        ...
        iterations = cls.cast(Rnn, int, thrownmessage="Invalid iterations.")
    ...
    def _split_to_microseconds(cls, floatstr, conversion, thrownmessage):
        ...
        intvalue = cls.cast(intpart, int, thrownmessage=thrownmessage)

        preconvertedvalue = cls.cast(floatpart, int, thrownmessage=thrownmessage)

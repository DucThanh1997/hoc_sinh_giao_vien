...
"""
    ...
    def test_33_options(self):
        ...
        #
        self.assertEqual(
            options(cc4, "des_crypt"), dict(deprecated=True, truncate_error=True)
        )
        ...
        self.assertEqual(
            ...
            dict(deprecated=True, truncate_error=True),  # unchanged though overidden
        )
        ...
        self.assertEqual(
            ...
            dict(vary_rounds=0.2),  # overridden from all__vary_rounds
        )

        self.assertEqual(
            options(cc4, "bsdi_crypt", "user"),
            dict(vary_rounds=0.2),  # unconfigured category inherits from default
        )

        self.assertEqual(
            options(cc4, "bsdi_crypt", "admin"),
            dict(vary_rounds=0.3, deprecated=True),  # deprecation set per-category
        )
    ...
    def test_disabled_hashes(self):
        ...
        self.assertRaisesRegex(
            ValueError, HASH_NOT_IDENTIFIED, ctx.is_enabled, h_dis_ref
        )

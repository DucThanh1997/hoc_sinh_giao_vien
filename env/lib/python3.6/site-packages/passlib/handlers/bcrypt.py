...
class _BcryptCommon(
    ...
    def _finalize_backend_mixin(mixin_cls, backend, dryrun):
        ...
        def assert_lacks_8bit_bug(ident):
            ...
            bug_hash = (
                ident.encode("ascii")
                + b"05$/OK.fbVrR/bpIqNJ5ianF.CE5elHaaO4EbggVDjb8P19RukzXSM3e"
            )
            ...
            correct_hash = (
                ident.encode("ascii")
                + b"05$/OK.fbVrR/bpIqNJ5ianF.Sa7shbm4.OzKpvFnX1pQLmQW96oUlCq"
            )
...
class _BcryptBackend(_BcryptCommon):
    ...
    def _calc_checksum(self, secret):
        ...
        assert (
            hash.startswith(config) and len(hash) == len(config) + 31
